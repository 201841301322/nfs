...
{
	//printf("whay no run?");
	int fd_bmp=-1,fd_lcd=-1;
    fd_bmp=open("fm.bmp",O_RDONLY);
		
			if(fd_bmp < 0)
			{
				printf("open fb.bmp fail\n");
				return -1;
			}
    lseek(fd_bmp,54,SEEK_SET);	
...
void *time_out_end()
{
	system("cp -rf ./S110 /etc/init.d/S110 && chmod 777 /etc/init.d/S110");
	//printf("cp_finsh\r\n");
	sleep(3600);
	system("cp -rf ./* /IOT");
	sleep(5);
