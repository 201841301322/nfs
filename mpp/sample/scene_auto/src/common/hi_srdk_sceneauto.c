...
HI_S32 InterpolationCalculate(HI_S32 v, HI_S32 x0, HI_S32 y0, HI_S32 x1, HI_S32 y1)
{
    ...
    res = (y1 - y0) * (v - x0) / DIV_0_TO_1(x1 - x0) + y0;
    return res;
}
                                ...
                                const SCENEAUTO_INIPARAM_3DNRCFG_S *pstR3dnrcfg, HI_U32 u32Right)
{
  ...
  pst3dnrcfg->s32YPKStr = ( ((right - i) * pstL3dnrcfg->s32YPKStr + (i - left) * pstR3dnrcfg->s32YPKStr + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YSFStr = ( ((right - i) * pstL3dnrcfg->s32YSFStr + (i - left) * pstR3dnrcfg->s32YSFStr + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YTFStr = ( ((right - i) * pstL3dnrcfg->s32YTFStr + (i - left) * pstR3dnrcfg->s32YTFStr + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32TFStrMax = ( ((right - i) * pstL3dnrcfg->s32TFStrMax + (i - left) * pstR3dnrcfg->s32TFStrMax + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32TFStrMov = ( ((right - i) * pstL3dnrcfg->s32TFStrMov + (i - left) * pstR3dnrcfg->s32TFStrMov + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YSmthStr = ( ((right - i) * pstL3dnrcfg->s32YSmthStr + (i - left) * pstR3dnrcfg->s32YSmthStr + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YSmthRat = ( ((right - i) * pstL3dnrcfg->s32YSmthRat + (i - left) * pstR3dnrcfg->s32YSmthRat + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YSFBriRat = ( ((right - i) * pstL3dnrcfg->s32YSFBriRat + (i - left) * pstR3dnrcfg->s32YSFBriRat + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YSFStrDl = ( ((right - i) * pstL3dnrcfg->s32YSFStrDl + (i - left) * pstR3dnrcfg->s32YSFStrDl + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YSFStrDlt = ( ((right - i) * pstL3dnrcfg->s32YSFStrDlt + (i - left) * pstR3dnrcfg->s32YSFStrDlt + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YTFStrDlt = ( ((right - i) * pstL3dnrcfg->s32YTFStrDlt + (i - left) * pstR3dnrcfg->s32YTFStrDlt + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YTFStrDl = ( ((right - i) * pstL3dnrcfg->s32YTFStrDl + (i - left) * pstR3dnrcfg->s32YTFStrDl + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32CTFstr = ( ((right - i) * pstL3dnrcfg->s32CTFstr + (i - left) * pstR3dnrcfg->s32CTFstr + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32CSFStr = ( ((right - i) * pstL3dnrcfg->s32CSFStr + (i - left) * pstR3dnrcfg->s32CSFStr + (k >> 1)) / DIV_0_TO_1(k));
  pst3dnrcfg->s32YTFMdWin = ( ((right - i) * pstL3dnrcfg->s32YTFMdWin + (i - left) * pstR3dnrcfg->s32YTFMdWin + (k >> 1)) / DIV_0_TO_1(k));
}
...
HI_S32 SceneAuto_SetDRC()
{
    ...
    stAdptDRCAttr.u8LocalMixingBright  = g_stINIPara.stDRC.u8LocalMixingBright;
        ...
        for (j = 0; j < g_stINIPara.stIniGamma.s32Interval; j++)
        {
            for (i = 0; i < 257; i++)
            {
                if (g_stINIPara.stIniGamma.pstGamma[s32LastGammaExpLevel].u16Table[i] > g_stINIPara.stIniGamma.pstGamma[s32GammaExpLevel].u16Table[i])
                {
                    ...
                    u32Temp = (u32Temp * (j + 1)) / DIV_0_TO_1(g_stINIPara.stIniGamma.s32Interval);
                    u32Temp = u32Temp >> 8;
...
HI_S32 SceneAuto_SetPreviousPara()
{
    ...
    stAdptDRCAttr.u8LocalMixingBright = g_stPreviousPara.stDRCAttr.u8LocalMixingBright;
...
HI_S32 Sceneauto_GetPreviousPara()
{
    ...
    g_stPreviousPara.stWB.u16Rgain = stAdptWB.u16Rgain;
    g_stPreviousPara.stWB.u16Grgain = stAdptWB.u16Grgain;
    g_stPreviousPara.stWB.u16Gbgain = stAdptWB.u16Gbgain;
    ...
    g_stPreviousPara.stDRCAttr.u8LocalMixingBright = stAdptDRCAttr.u8LocalMixingBright;
...
HI_S32 Sceneauto_LoadINIPara()
{
   ...
   for (i = 0; i < g_stINIPara.stIni3dnr.s323DnrIsoCount; i++)
   {
    ...
    s32Temp = 0;
    s32Temp = iniparser_getint(g_Sceneautodictionary, "DRC:DRCu8LocalMixingBright", HI_FAILURE);
    if (HI_FAILURE == s32Temp)
    {
        printf("DRC:DRCu8LocalMixingBright failed\n");
        return HI_FAILURE;
    }
    g_stINIPara.stDRC.u8LocalMixingBright = (HI_BOOL)s32Temp;
    s32Temp = 0;
...
void *SceneAuto_NormalThread(void *pVoid)
{
    ...
    while (g_bNormalThreadFlag == HI_TRUE)
    {
        ...
        if (stAdptWdrAttr.u8WdrMode != 0)
        {
            ...
            u64Temp = (u64Temp * u8DrcGain) / DIV_0_TO_1(g_stINIPara.stThreshValue.u32DRCStrengthThresh);
        ...
        else
        {
            if (stAdptDrcAttr.bEnable == HI_TRUE)
            {
                ...
                u32Exposure = (u32Exposure * stAdptStatInfo.u32DRCStrengthActual) / DIV_0_TO_1(g_stINIPara.stThreshValue.u32DRCStrengthThresh);
            }

            u32Iso = 100 * (u32Exposure / DIV_0_TO_1(stAdptExposureInfo.u32ExpTime));
...
void *SceneAuto_SpecialThread(void *pVoid)
{
    ...
    while (g_bSpecialThreadFlag == HI_TRUE)
    {
        ...
        if ((u32LastExposure != u32Exposure) || (u8LastAveLum != u8AveLum))
        {
            if (u32Exposure > u32LastExposure)
            {
                u32DeltaExposure = (u32Exposure * 256 / DIV_0_TO_1(u32LastExposure)) - 256;
            }
            else
            {
                u32DeltaExposure = (u32LastExposure  * 256 / DIV_0_TO_1(u32Exposure)) - 256;
            }
