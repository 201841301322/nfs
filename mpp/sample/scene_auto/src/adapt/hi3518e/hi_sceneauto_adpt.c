...
int IVE_DetectInit(IVE_MD* pstIveMd)
{
    ...
    if ((pstIveMd->stImg1.u16Stride[0] & (IVE2_ALIGN - 1)) != 0)
    {
        for (i = 4; i < 255; i++)
        {
            if (u32Size == (u32Size / DIV_0_TO_1(i * IVE2_ALIGN)) * (i * IVE2_ALIGN))
            { break; }
        }
        if (i == 255)
        {
        ...
        pstIveMd->stImg1.u16Height = u32Size / DIV_0_TO_1(i * IVE2_ALIGN);
    }
...
HI_S32 EncSetMbQp(int EncChn,int *ps32Hist,int width,int height,int u32HistSize)
{
	...
	int s32Ret;
	j=0;
	f32factor=1.0/DIV_0_TO_1(u32HistSize);
...
HI_S32 EncSetMbQpSec(int EncChn,int *ps32Hist,int width,int height,int u32HistSize)
{
    ...
    for(i = 0;i < 25;i++)
    {
        percentmadi[i] = StaticMadi[i]*100 /DIV_0_TO_1(mbnum);
    }
    ...
    for(i = 1;i < 25;i++)
    {
        ...
        totalpercentmadi[i] =  totalmadi[i] * 100 /DIV_0_TO_1(mbnum);
    }
    for(i = 1;i < 25;i++)
    {
...
HI_S32 CommSceneautoGetSharpen(HI_S32 s32IspDev, ADPT_SCENEAUTO_SHARPEN_S *pstAdptSharpen)
{
	...
	for (i = 0; i < 16; i++)
    {
        pstAdptSharpen->abEnLowLumaShoot[i] = stSharpenAttr.stAuto.abEnLowLumaShoot[i];
		pstAdptSharpen->u8SharpenD[i] = stSharpenAttr.stAuto.au8SharpenD[i];
		pstAdptSharpen->u8SharpenUd[i] = stSharpenAttr.stAuto.au8SharpenUd[i];
		pstAdptSharpen->u8OverShoot[i] = stSharpenAttr.stAuto.au8OverShoot[i];
		pstAdptSharpen->u8UnderShoot[i] = stSharpenAttr.stAuto.au8UnderShoot[i];
		pstAdptSharpen->u8TextureNoiseThd[i] = stSharpenAttr.stAuto.au8TextureNoiseThd[i];
		pstAdptSharpen->u8EdgeNoiseThd[i] = stSharpenAttr.stAuto.au8EdgeNoiseThd[i]; 
		}
    return HI_SUCCESS;
}
...
HI_S32 CommSceneautoSetSharpen(HI_S32 s32IspDev, const ADPT_SCENEAUTO_SHARPEN_S *pstAdptSharpen)
{
    ...
    for (i = 0; i < 16; i++)
    {
        stSharpenAttr.stAuto.abEnLowLumaShoot[i] = pstAdptSharpen->abEnLowLumaShoot[i];
        stSharpenAttr.stAuto.au8SharpenD[i] = pstAdptSharpen->u8SharpenD[i];
        stSharpenAttr.stAuto.au8SharpenUd[i] = pstAdptSharpen->u8SharpenUd[i];
		stSharpenAttr.stAuto.au8OverShoot[i] = pstAdptSharpen->u8OverShoot[i];
		stSharpenAttr.stAuto.au8UnderShoot[i] = pstAdptSharpen->u8UnderShoot[i];
		stSharpenAttr.stAuto.au8TextureNoiseThd[i] = pstAdptSharpen->u8TextureNoiseThd[i];
		stSharpenAttr.stAuto.au8EdgeNoiseThd[i] = pstAdptSharpen->u8EdgeNoiseThd[i];		
    }
...
HI_S32 CommSceneautoGetDP(HI_S32 s32IspDev, ADPT_SCENEAUTO_DP_S *pstAdptDP)
{
    ...
    for (i = 0; i < 16; i++)
    {
        pstAdptDP->u16Slope[i] = stDPAttr.stAuto.au16Slope[i];
    }
    return HI_SUCCESS;
}
...
HI_S32 CommSceneautoSetDP(HI_S32 s32IspDev, const ADPT_SCENEAUTO_DP_S *pstAdptDP)
{
    ...
    for (i = 0; i < 16; i++)
    {
        stDPAttr.stAuto.au16Slope[i] = pstAdptDP->u16Slope[i];
    }
...
HI_S32 CommSceneautoGetWB(HI_S32 s32IspDev, ADPT_SCENEAUTO_WB_S *pstAdptWB)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ISP_DEV IspDev = s32IspDev;
    ...
    if (HI_SUCCESS != s32Ret)
    {
     ...
     pstAdptWB->bManulEnable  = stWBAttr.enOpType;
     pstAdptWB->u16Bgain  = stWBAttr.stManual.u16Bgain;
     pstAdptWB->u16Gbgain = stWBAttr.stManual.u16Gbgain;
     pstAdptWB->u16Grgain = stWBAttr.stManual.u16Grgain;
     pstAdptWB->u16Rgain  = stWBAttr.stManual.u16Rgain;
        
    return HI_SUCCESS;
}

HI_S32 CommSceneautoSetWB(HI_S32 s32IspDev, const ADPT_SCENEAUTO_WB_S *pstAdptWB)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ISP_DEV IspDev = s32IspDev;
    ISP_WB_ATTR_S stWBAttr;
    ...
    if (HI_SUCCESS != s32Ret)
    {
     ...
     stWBAttr.enOpType = pstAdptWB->bManulEnable;
     stWBAttr.stManual.u16Bgain  = pstAdptWB->u16Bgain;
     stWBAttr.stManual.u16Gbgain = pstAdptWB->u16Gbgain;
     stWBAttr.stManual.u16Grgain = pstAdptWB->u16Grgain;
     stWBAttr.stManual.u16Rgain  = pstAdptWB->u16Rgain;
...
HI_S32 CommSceneautoGetRgbirParam(HI_S32 s32IspDev, ADPT_SCENEAUTO_RGBIRPARAM_S *pstAdptRgbirPara)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ISP_RGBIR_ATTR_S stRgbirAttr;
    ISP_RGBIR_CTRL_S stRgbirCtrl;

    s32Ret = HI_MPI_ISP_GetRgbirAttr(s32IspDev,  &stRgbirAttr);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_ISP_GetRgbirAttr failed\n");
        return HI_FAILURE;
    }
    s32Ret = HI_MPI_ISP_GetRgbirCtrl(s32IspDev,  &stRgbirCtrl);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_ISP_GetRgbirCtrl failed\n");
        return HI_FAILURE;
    }

    pstAdptRgbirPara->bEnable = stRgbirAttr.bEnable;
    pstAdptRgbirPara->bRemovelEn = stRgbirCtrl.bRemovelEn;
        
    return HI_SUCCESS;
}

HI_S32 CommSceneautoSetRgbirParam(HI_S32 s32IspDev, ADPT_SCENEAUTO_RGBIRPARAM_S *pstAdptRgbirPara)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ISP_RGBIR_ATTR_S stRgbirAttr;
    ISP_RGBIR_CTRL_S stRgbirCtrl;
    ...
    stRgbirAttr.bEnable = pstAdptRgbirPara->bEnable;
    stRgbirCtrl.bRemovelEn = pstAdptRgbirPara->bRemovelEn;

    s32Ret = HI_MPI_ISP_SetRgbirAttr(s32IspDev,  &stRgbirAttr);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_ISP_SetRgbirAttr failed\n");
        return HI_FAILURE;
    }
    s32Ret = HI_MPI_ISP_SetRgbirCtrl(s32IspDev,  &stRgbirCtrl);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_ISP_SetRgbirCtrl failed\n");
        return HI_FAILURE;
    }
...
HI_S32 CommSceneautoGetDRCAttr(HI_S32 s32IspDev, ADPT_SCENEAUTO_DRCATTR_S *pstAdptDRCAttr)
{
    ...
    pstAdptDRCAttr->u8Strength = stDRCAttr.stManual.u8Strength;
    pstAdptDRCAttr->u16BrightGainLmt = stDRCAttr.u16BrightGainLmt;
    pstAdptDRCAttr->u16DarkGainLmtC = stDRCAttr.u16DarkGainLmtC;
    pstAdptDRCAttr->u16DarkGainLmtY = stDRCAttr.u16DarkGainLmtY;
    pstAdptDRCAttr->u8Asymmetry = stDRCAttr.u8Asymmetry;
    pstAdptDRCAttr->u8LocalMixingBright = stDRCAttr.stManual.u8LocalMixingBright;
    pstAdptDRCAttr->u8LocalMixingDark = stDRCAttr.stManual.u8LocalMixingDark;
	pstAdptDRCAttr->u8LocalMixingThres = stDRCAttr.u8LocalMixingThres;
	pstAdptDRCAttr->u8RangeVar = stDRCAttr.u8RangeVar;
	pstAdptDRCAttr->u8SecondPole = stDRCAttr.u8SecondPole;
	pstAdptDRCAttr->u8SpatialVar= stDRCAttr.u8SpatialVar;
	pstAdptDRCAttr->u8Stretch = stDRCAttr.u8Stretch;
...
HI_S32 CommSceneautoSetDRCAttr(HI_S32 s32IspDev, const ADPT_SCENEAUTO_DRCATTR_S *pstAdptDRCAttr)
{
	...
	stDRCAttr.stManual.u8Strength           = pstAdptDRCAttr->u8Strength;
    stDRCAttr.u16BrightGainLmt              = pstAdptDRCAttr->u16BrightGainLmt;
    stDRCAttr.u16DarkGainLmtC               = pstAdptDRCAttr->u16DarkGainLmtC;
    stDRCAttr.u16DarkGainLmtY               = pstAdptDRCAttr->u16DarkGainLmtY;
    stDRCAttr.u8Asymmetry                   = pstAdptDRCAttr->u8Asymmetry;
    stDRCAttr.stManual.u8LocalMixingBright = pstAdptDRCAttr->u8LocalMixingBright;
    stDRCAttr.stManual.u8LocalMixingDark    = pstAdptDRCAttr->u8LocalMixingDark;
	stDRCAttr.u8LocalMixingThres            = pstAdptDRCAttr->u8LocalMixingThres;
	stDRCAttr.u8RangeVar                    = pstAdptDRCAttr->u8RangeVar;
	stDRCAttr.u8SecondPole                  = pstAdptDRCAttr->u8SecondPole;
	stDRCAttr.u8SpatialVar                  = pstAdptDRCAttr->u8SpatialVar;
	stDRCAttr.u8Stretch                     = pstAdptDRCAttr->u8Stretch;
...
HI_S32 CommSceneautoGetExposureInfo(HI_S32 s32IspDev, ADPT_SCENEAUTO_EXPOSUREINFO_S *pstAdptExposureInfo)
{
    ...
    pstAdptExposureInfo->u16RgbirGain = stIspInnerStateInfo.u32RgbirGain;
...
HI_S32 CommSceneautoGet3DNRAttr(HI_S32 s32VpssGrp, ADPT_SCENEAUTO_3DNR_ATTR_S * pstAdpt3dnrAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    VPSS_GRP VpssGrp = s32VpssGrp;
    VPSS_NR_PARAM_U stVppnrbex;

    s32Ret = HI_MPI_VPSS_GetNRParam(VpssGrp, &stVppnrbex);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_VPSS_GetNRParam failed\n");
    }
    
    pstAdpt3dnrAttr->s32YPKStr    = stVppnrbex.stNRParam_V1.s32YPKStr;
        
    pstAdpt3dnrAttr->s32YSFStr    = stVppnrbex.stNRParam_V1.s32YSFStr;
    pstAdpt3dnrAttr->s32YTFStr    = stVppnrbex.stNRParam_V1.s32YTFStr;
    pstAdpt3dnrAttr->s32TFStrMax  = stVppnrbex.stNRParam_V1.s32TFStrMax;
    pstAdpt3dnrAttr->s32TFStrMov  = stVppnrbex.stNRParam_V1.s32TFStrMov;

    pstAdpt3dnrAttr->s32YSFStrDlt = stVppnrbex.stNRParam_V1.s32YSFStrDlt;
    pstAdpt3dnrAttr->s32YSFStrDl  = stVppnrbex.stNRParam_V1.s32YSFStrDl;
    pstAdpt3dnrAttr->s32YTFStrDlt = stVppnrbex.stNRParam_V1.s32YTFStrDlt;
    pstAdpt3dnrAttr->s32YTFStrDl  = stVppnrbex.stNRParam_V1.s32YTFStrDl;
        
    pstAdpt3dnrAttr->s32YSmthStr  = stVppnrbex.stNRParam_V1.s32YSmthStr;
    pstAdpt3dnrAttr->s32YSmthRat  = stVppnrbex.stNRParam_V1.s32YSmthRat;
    pstAdpt3dnrAttr->s32YSFBriRat = stVppnrbex.stNRParam_V1.s32YSFBriRat;

    pstAdpt3dnrAttr->s32CSFStr    = stVppnrbex.stNRParam_V1.s32CSFStr;
    pstAdpt3dnrAttr->s32CTFstr    = stVppnrbex.stNRParam_V1.s32CTFstr;
    
    pstAdpt3dnrAttr->s32YTFMdWin  = stVppnrbex.stNRParam_V1.s32YTFMdWin;
...
HI_S32 CommSceneautoSet3DNRAttr(HI_S32 s32VpssGrp, ADPT_SCENEAUTO_3DNR_ATTR_S *pstAdpt3dnrAttr)
{
    ...
    stVppnrbex.stNRParam_V1.s32YPKStr      = pstAdpt3dnrAttr->s32YPKStr;
        
    stVppnrbex.stNRParam_V1.s32YSFStr      = pstAdpt3dnrAttr->s32YSFStr;
    stVppnrbex.stNRParam_V1.s32YTFStr      = pstAdpt3dnrAttr->s32YTFStr;
    stVppnrbex.stNRParam_V1.s32TFStrMax    = pstAdpt3dnrAttr->s32TFStrMax;
    stVppnrbex.stNRParam_V1.s32TFStrMov    = pstAdpt3dnrAttr->s32TFStrMov;

    stVppnrbex.stNRParam_V1.s32YSFStrDlt   = pstAdpt3dnrAttr->s32YSFStrDlt;  
    stVppnrbex.stNRParam_V1.s32YSFStrDl    = pstAdpt3dnrAttr->s32YSFStrDl;
    stVppnrbex.stNRParam_V1.s32YTFStrDlt   = pstAdpt3dnrAttr->s32YTFStrDlt;
    stVppnrbex.stNRParam_V1.s32YTFStrDl    = pstAdpt3dnrAttr->s32YTFStrDl;
        
    stVppnrbex.stNRParam_V1.s32YSmthStr    = pstAdpt3dnrAttr->s32YSmthStr;
    stVppnrbex.stNRParam_V1.s32YSmthRat    = pstAdpt3dnrAttr->s32YSmthRat;
    stVppnrbex.stNRParam_V1.s32YSFBriRat   = pstAdpt3dnrAttr->s32YSFBriRat;

    stVppnrbex.stNRParam_V1.s32CSFStr      = pstAdpt3dnrAttr->s32CSFStr;
    stVppnrbex.stNRParam_V1.s32CTFstr      = pstAdpt3dnrAttr->s32CTFstr;

    stVppnrbex.stNRParam_V1.s32YTFMdWin    = pstAdpt3dnrAttr->s32YTFMdWin;
    
    s32Ret = HI_MPI_VPSS_SetNRParam(VpssGrp, &stVppnrbex);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_VPSS_SetNRParam failed\n");
    }
