    ...
    const char * val)
{
    ...
    if (key!=NULL) {
        ...
        strncpy(longkey, sec, 2*ASCIILINESZ);
        longkey[2*ASCIILINESZ]  = '\0';
    }
...
void iniparser_dump_ini(const dictionary * d, FILE * f)
{
    ...
    for (i=0 ; i<nsec ; i++)
    {
        ...
        (void)fseek(f, 0-(50-strlen(secname)), SEEK_CUR);
#elif HI_OS_TYPE == HI_OS_LINUX
        (void)fseek(f, 0-(49-strlen(secname)), SEEK_CUR);
#endif
        fprintf(f, "%c", ']');
        ...
        snprintf(keym, sizeof(keym), "%s:", secname);
        for (j=0 ; j<d->size ; j++) {
            if (d->key[j]==NULL)
                continue ;
        ...
        else
        {
            ...
            else
            {
                    ...
                    || (sscanf(where, "[%[^]]", sec)==1))/*[sec];comment*/
                {
                    /* Valid section name */
                    key[0] = 0;
                    strncpy(sec, strlwc(sec), KVCSIZE);
                    sec[KVCSIZE] = '\0';
                ...
                else /*key = value ;comment*/
                {
                    ...
                    if (sscanf(where, "%[^=] = \"%[^\"]\" %[^\n]", key, val,
                       ...
                       ||  sscanf(where, "%[^=] = ", key) == 1)
                    {
                        strncpy(key, strlwc(strcrop(key)), KVCSIZE);
                        key[KVCSIZE] = '\0';
                        /*
                        ...
                        if (!strcmp(val, "\"\"") || !strcmp(val, "''")) {
                            ...
                            strncpy(val, strcrop(val), KVCSIZE);
                            val[KVCSIZE] = '\0';
                        }
